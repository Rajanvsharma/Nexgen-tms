...
export default function CarrierTopbar({ title, subtitle }: Props) {
  ...
  function logout() { clearAuth(); router.replace('/carrier-login'); }
  ...
  return (
    <div style={{ height: 56, background: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', padding: '0 24px', gap: 16, flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button onClick={logout} style={{ padding: '6px 14px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
          Logout
        </button>
      </div>
    </div>
