...
import { FileText, Download, Truck, Send, CheckCircle } from 'lucide-react';
...
export default function DocumentsPage() {
  ...
  async function sendRateCon(loadId: string) {
    setSending(loadId);
    try {
      const { data } = await api.post(`/documents/loads/${loadId}/send-rate-confirmation`);
      showToast(data.simulated
        ? 'Rate confirmation sent (simulated — configure SMTP for real emails). Sign link: ' + data.signUrl
        : `Rate confirmation emailed to carrier. Sign link: ${data.signUrl}`);
      await loadData();
    } catch (err: unknown) {
  ...
  return (
    <>
      {toast && (
        <div className="fixed bottom-4 right-4 z-50 bg-gray-900 text-white text-sm px-4 py-3 rounded-lg shadow-lg max-w-sm break-all">
          {toast}
        </div>
      )}
          ...
          {loading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Load #', 'Customer', 'Route', 'Carrier', 'Status', 'Rate Confirmation', 'Send Rate Con', 'Bill of Lading'].map((h) => (
              ...
              <tbody className="divide-y divide-gray-100">
                {loads.map((l) => (
                  <tr key={l.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {l.carrier ? (
                        <Button size="sm" variant="outline" disabled={sending === l.id}
                          onClick={() => sendRateCon(l.id)}>
                          <Send className="h-3 w-3 mr-1" />
                          {sending === l.id ? 'Sending…' : 'Email Carrier'}
                        </Button>
                      ) : (
                        <span className="text-xs text-gray-400 italic">No carrier</span>
                      )}
                    </td>
                    <td className="px-4 py-3">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {docs.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Type', 'Load', 'Filename', 'Signed', 'Generated By', 'Date'].map((h) => (
                    <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3">
                      {d.signedAt ? (
                        <span className="flex items-center gap-1 text-xs text-green-600 font-medium">
                          <CheckCircle className="h-3 w-3" />{d.signerName}
                        </span>
                      ) : (
