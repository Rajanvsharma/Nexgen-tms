...
interface Doc { id:string; type:string; filename:string; loadId:string; loadNumber:string; signedAt:string|null; createdAt:string; }
interface POD { id:string; filename:string; fileUrl:string; podType:string; notes:string|null; fileSize:number|null; loadId:string; loadNumber:string; createdAt:string; }
...
export default function CarrierDocumentsPage() {
  ...
  useEffect(() => {
    api.get('/carrier-portal/documents').then(r => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <CarrierTopbar title="Documents" subtitle="Rate confirmations, BOLs & PODs" />
      <div style={{ flex:1, overflowY:'auto', padding:'24px 28px' }}>

        <div style={{ marginBottom:20 }}>
          <h1 style={{ fontSize:22, fontWeight:800, color:'#0f172a', margin:0 }}>Documents</h1>
          <p style={{ color:'#64748b', fontSize:13, marginTop:4 }}>{combined.length} documents across all loads</p>
        </div>
          ...
          {loading ? (
            ...
            <div style={{ padding:'48px 24px', textAlign:'center' }}>
              ...
              <div style={{ color:'#94a3b8', fontSize:14 }}>
                {combined.length === 0 ? 'No documents yet. Rate confirmations and PODs will appear here once loads are assigned.' : 'No documents in this category.'}
              </div>
            </div>
          ) : (
            <div>
                ...
                return (
                  <div key={d.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 18px', borderBottom:'1px solid #f1f5f9' }}>
                    ...
                    {d.fileUrl ? (
                      ...
                      <span style={{ padding:'6px 14px', background:'#f8fafc', border:'1px solid #e2e8f0', borderRadius:7, fontSize:12, color:'#94a3b8', flexShrink:0 }}>View in portal</span>
                    )}
                  </div>
                );
              })}
